...
function resolveFormattingOptions(
  ...
  return {
    maxTranscriptChars: Math.max(
      1,
      Math.floor(formatting?.maxTranscriptChars ?? DEFAULT_MAX_TRANSCRIPT_CHARS)
    ),
    maxPartChars: Math.max(
      1,
      Math.floor(formatting?.maxPartChars ?? DEFAULT_MAX_PART_CHARS)
    ),
    deterministicSummaryMaxChars: Math.max(
      1,
      Math.floor(
        formatting?.deterministicSummaryMaxChars ??
...
function formatSummaryPart(
  part: LanguageModelV3Message["content"][number],
  maxPartChars: number
): string {
...
export function buildDeterministicSummary(
  ...
  let remaining =
    resolvedFormatting.deterministicSummaryMaxChars - lines[0].length;
  ...
  for (const message of messages) {
    ...
    const candidate = `- ${formatMessageForSummary(
      message,
      resolvedFormatting
    ).replace(/\s+/g, " ")}`;
    const clipped = clipText(
      candidate,
      Math.min(remaining, resolvedFormatting.maxPartChars)
    );
    lines.push(clipped);

...
function buildReminder(options: {
  ...
  if (workingTokenBudget !== undefined && budgetScopedTokens !== undefined) {
    const pct = formatPercent(budgetScopedTokens, workingTokenBudget);
    parts.push(
      `${budgetLabel ?? "Working context"}: ${pct}% (~${formatNumber(budgetScopedTokens)}/${formatNumber(workingTokenBudget)} tokens).`
    );
  }

  if (rawContextWindow !== undefined) {
    const pct = formatPercent(estimatedRequestTokens, rawContextWindow);
    parts.push(
      `Model window: ${pct}% (~${formatNumber(estimatedRequestTokens)}/${formatNumber(rawContextWindow)} tokens).`
    );
  }

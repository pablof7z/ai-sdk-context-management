import type { LanguageModelV3CallOptions, LanguageModelV3Message, LanguageModelV3Prompt, LanguageModelV3ToolResultOutput } from "@ai-sdk/provider";
...
export interface ReminderProvider<TData = unknown, TSnapshot = unknown> {
    type: string;
    fullInterval?: number;
    placement?: ReminderPlacement | ((context: ReminderProviderContext<TData>) => ReminderPlacement);
    snapshot(data: TData | undefined, context: ReminderProviderContext<TData>): TSnapshot | Promise<TSnapshot>;
    renderFull(snapshot: TSnapshot, data: TData | undefined, context: ReminderProviderContext<TData>): ReminderDescriptor | null | Promise<ReminderDescriptor | null>;
    renderDelta?(previous: TSnapshot, current: TSnapshot, data: TData | undefined, context: ReminderProviderContext<TData>): ReminderProviderDeltaResult | Promise<ReminderProviderDeltaResult>;
}
...
export type ReminderPlacementPolicy<TData = unknown> = (context: ReminderPlacementPolicyContext<TData>) => ReminderPlacement;
...
export type ContextManagementTelemetryEvent = ContextManagementRuntimeStartEvent | ContextManagementStrategyCompleteEvent | ContextManagementToolExecuteStartEvent | ContextManagementToolExecuteCompleteEvent | ContextManagementToolExecuteErrorEvent | ContextManagementRuntimeCompleteEvent | ContextManagementCalibrationEvent;
...
export interface ContextManagementRuntime {
    prepareRequest(options: PrepareContextManagementRequestOptions): Promise<ContextManagementPreparedRequest>;
...
export interface CompactionToolStrategyOptions {
    shouldCompact?: (args: CompactionShouldCompactArgs) => Promise<boolean> | boolean;
    onCompact?: (args: CompactionOnCompactArgs) => Promise<string> | string;
    preserveRecentMessages?: number;
...
export type ContextManagementStrategyPayload = SlidingWindowStrategyPayload | ToolResultDecayStrategyPayload | ScratchpadStrategyPayload | SummarizationStrategyPayload | RemindersStrategyPayload | AnthropicPromptCachingStrategyPayload | CompactionToolStrategyPayload | PinnedMessagesStrategyPayload | CustomStrategyPayload;

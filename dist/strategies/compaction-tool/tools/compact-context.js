...
import { extractCompactionSummaryRange, extractMessageAnchor, getMessageTextContent, isCompactionSummaryMessage, normalizeCompactionText, } from "../shared.js";
...
function isEligibleManualCompactionMessage(message) {
    return message.role === "user" || message.role === "assistant";
}
function getProtectedTailStartIndex(messages) {
...
function buildVisibleCandidates(messages) {
    ...
    return messages.flatMap((message, promptIndex) => {
        ...
        const normalizedText = getMessageTextContent(message);
        if (!normalizedText) {
            return [];
        }
        const summaryRange = isCompactionSummaryMessage(message)
            ? extractCompactionSummaryRange(message)
            : undefined;
        const directAnchor = summaryRange
            ? undefined
            : extractMessageAnchor(message);
...
function findUniqueAnchorMatch(candidates, anchorText) {
    if (!anchorText) {
        return undefined;
    }
...
function buildQueuedEditId(toolCallId) {
    const suffix = typeof toolCallId === "string" && toolCallId.length > 0
        ? toolCallId
        : "compact-context";
    return `compact:${Date.now()}:${suffix}`;
}
function countCompactedMessages(messages, fromPromptIndex, toPromptIndex) {
    let count = 0;
    ...
    return tool({
        ...
        execute: async (input, executeOptions) => {
            ...
            const steeringMessage = typeof input.guidance === "string"
                ? input.guidance.trim()
                : undefined;
            const candidates = buildVisibleCandidates((executeOptions.messages ?? []));
            if (candidates.length === 0) {
                return buildError("No eligible historical user or assistant messages are available for compaction.");
            }
            ...
            const compactedMessageCount = countCompactedMessages((executeOptions.messages ?? []), startCandidate.promptIndex, endCandidate.promptIndex);
            const queuedEdit = {

...
function buildReminderBlock(options) {
    ...
    if (forced) {
        lines.push("CRITICAL: Context is nearly full. You MUST:", "1. Record side-effect actions in your scratchpad (entries and/or notes)", "2. Set keepLastMessages to trim old messages (e.g. 5-10)", "3. Add completed tool call IDs to omitToolCallIds", "Failure to free context will result in an error.");
    }
        ...
        if ((currentState.entries === undefined || Object.keys(currentState.entries).length === 0)
            ...
            lines.push("Suggested entry names for this run: objective, thesis, findings, side-effects, next-steps. Use any keys that fit the work.");
        }

...
export function buildContextManagementSystemMessage(content, metadata = { type: "reminder" }) {
    return {
        role: "system",
        content,
...
export function buildContextManagementUserOverlayMessage(content, metadata = { type: "reminder-overlay" }) {
    return {
        role: "user",
...
function extractScratchpadTurnSpans(prompt) {
    const spans = [];
    let openSpan;
...
function flattenScratchpadTurnSpans(prompt, spans) {
    return spans.flatMap((span) => clonePrompt(prompt.slice(span.startIndex, span.endIndex)));
}
function getUnmatchedTurnTail(turns) {
    const latestTurn = turns.at(-1);
    return latestTurn && latestTurn.assistant === undefined ? [latestTurn] : [];
}
function getUnmatchedTurnSpanTail(spans) {
    const latestSpan = spans.at(-1);
    return latestSpan && !latestSpan.hasAssistantText ? [latestSpan] : [];
}
function removeScratchpadUseNotices(prompt) {
    return clonePrompt(prompt).filter((message) => !isScratchpadUseNoticeMessage(message));
}
function compactScratchpadTurns(prompt, preserveTurns) {
...
function replaceScratchpadExchangesWithNotices(prompt, notices) {
    if (notices.size === 0) {
        return clonePrompt(prompt);
    }
    const result = [];
...
export function projectScratchpadPrompt(prompt, options) {
    ...
    const scratchpadNotices = new Map();
    ...
    const promptWithoutNotices = replaceScratchpadExchangesWithNotices(removeScratchpadUseNotices(prompt), scratchpadNotices);
    ...
    if (noticeExchange?.callMessageIndex !== undefined) {
        const promptWithActiveNotice = replaceScratchpadExchangesWithNotices(promptWithoutNotices, new Map([[options.notice.toolCallId, options.notice.description]]));
    ...
    const preTurnCount = Math.min(Math.max(0, Math.floor(options.notice.rawTurnCountAtCall)), turnSpans.length);
